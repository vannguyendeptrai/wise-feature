  ...
  return (
    <>
      <div>
        ...
        <h3 class="flex items-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">Added to deposit: ${deposit.value}<span class="bg-blue-100 text-blue-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-3">+{deposit.point}</span></h3>
        {
          deposit.stage == "In Progress" ? (
            <div class="p-1 mb-4 text-sm text-yellow-700 bg-yellow-100 rounded-lg dark:bg-yellow-200 dark:text-yellow-800" role="alert">
              <span class="font-medium">Stage:</span> {deposit.stage}
            </div>
          ) :
            <div class="p-1 mb-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800" role="alert">
              <span class="font-medium">Stage:</span> {deposit.stage}
            </div>
        }
      </div>
